module.exports.websocket = async (pool, schema, table) => {
  const client = await pool.connect();
  console.log(`Initializing ${schema}.${table} table...`);
  try {
...
module.exports.chatroom = async (pool, schema, table) => {
  const client = await pool.connect();
  console.log(`Initializing ${schema}.${table} table...`);
  try {
...
module.exports.user = async (pool, schema, table) => {
  const client = await pool.connect();
  console.log(`Initializing ${schema}.${table} table...`);
  try {
...
module.exports.message = async (pool, schema, table) => {
  const client = await pool.connect();
  console.log(`Initializing ${schema}.${table} table...`);
  try {
